...
type ModalData = {
  title: string;
  description: string;
  mediaSrc: string;        // Шлях до картинки або відео
  mediaType: 'image' | 'video'; // Тип медіа
  link?: string;           // (Опціонально) посилання на профіль
  linkText?: string;       // (Опціонально) текст кнопки
};
...
const modalContent: Record<string, ModalData> = {
  waterpolo: {
    title: "Waterpolo 🤽‍♂️",
    description: "Professional career spanning over 10 years. Multiple champion of Ukraine, team captain. This sport taught me discipline, teamwork, and how to stay afloat in difficult situations.",
    mediaSrc: "/img/waterpolo-card.jpeg",
    mediaType: "image"
  },
  university: {
    title: "Lviv Polytechnic 🎓",
    description: "Master's degree in Applied Mathematics. Specialized in Artificial Intelligence systems. My diploma thesis focused on Neural Networks optimization.",
    mediaSrc: "/img/uni-card.jpg",
    mediaType: "image"
  },
  typing: {
    title: "Fast Typing ⌨️",
    description: "I believe that typing speed directly correlates with thought process efficiency in coding. I use Monkeytype to constantly improve my accuracy and speed.",
    mediaSrc: "/img/keyb.jpg",
    mediaType: "image",
    link: "https://monkeytype.com/profile/Martyyy46",
    linkText: "Visit Monkeytype Profile"
  },
  chess: {
    title: "Chess ♟️",
    description: "Chess is my mental gym. It helps me improve pattern recognition and strategic planning. I play daily on Chess.com to keep my mind sharp and ready for complex algorithmic challenges.",
...
const LifeHobbies = () => {
  ...
  // Знаходимо поточні дані хобі для стилізації (колір)
  const currentHobby = activeModal ? hobbies.find(h => h.id === activeModal) : null;

  return (
    <>
      <div ref={containerRef} className="relative h-[300vh] w-full font-aes">
        <div className="sticky top-0 h-screen flex items-center justify-center overflow-hidden">
          ...
          <motion.div 
            style={{ opacity, scale }} 
            className="relative z-10 max-w-6xl w-full mx-auto px-6"
          >
          ...
          {activeModal && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-center gap-8 md:gap-12 max-w-6xl w-full pointer-events-none">
                ...
                {/* --- ЛІВА ЧАСТИНА: МЕДІА (ФОТО АБО ВІДЕО) --- */}
                <motion.div
                ...
                >
                    {modalContent[activeModal].mediaType === 'video' ? (
                        // ВІДЕО-ПЛЕЄР (GIF STYLE)
                        <div className="
                            relative overflow-hidden
                            w-full max-w-[600px] md:w-[500px]
                        ...
                        ">
                            ...
                            {/* Overlay для кінематографічності */}
                ...
                <motion.div
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="pointer-events-auto relative w-full md:max-w-lg 
                  bg-white/90 dark:bg-gray-900/80 backdrop-blur-2xl 
                  border border-white/20 dark:border-white/10
                  p-8 md:p-12 rounded-[40px] shadow-2xl flex flex-col justify-center"
                >
                  <button 
                    onClick={() => setActiveModal(null)}
                  ...
                  {/* Декоративна лінія з кольором хобі */}
                  <div 
                  ...
                  {/* КНОПКА ПОСИЛАННЯ (Тільки якщо є лінк) */}
                  {modalContent[activeModal].link && (
                    <a 
                      ...
                      "
                      style={{ 
                          backgroundColor: currentHobby?.color || 'black',
                          boxShadow: `0 10px 30px -10px ${currentHobby?.color}60`
                      }}
                    >
